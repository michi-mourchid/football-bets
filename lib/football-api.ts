...
export type LeagueId =
  (typeof SUPPORTED_LEAGUES)[keyof typeof SUPPORTED_LEAGUES];
...
/**
 * Utility: Remove duplicate matches based on ID
 */
function deduplicateMatches(matches: FlashscoreMatch[]): FlashscoreMatch[] {
...
export async function fetchAllMatchesByLeague(leagueId: number) {
  const url = new URL(
    `${FOOTBALL_API_BASE}/football-get-all-matches-by-league`,
  );
  url.searchParams.set("leagueid", leagueId.toString());
...
export async function fetchMatchesByDate(
  ...
  const all = await Promise.all(
    leaguesToFetch.map((lid) => fetchAllMatchesByLeague(lid)),
  );
  ...
  const mappedMatches = matches.map((m) => ({
    ...
    status:
      m.status?.reason?.short ?? (m.status?.started ? "LIVE" : "SCHEDULED"),
  ...
  return deduplicateMatches(mappedMatches);
}
 ...
 */
export async function fetchMatchScoreById(
  eventId: string,
...
export async function fetchMatchById(
  ...
  // Return single match (no need to deduplicate, but consistency check)
  return match;
}
